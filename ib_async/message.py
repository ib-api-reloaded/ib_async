class MessageId:
    ...
    @staticmethod
    def to_protobuf(legacy_id: int) -> int:
        """
        Converts a legacy outgoing ID to its Protobuf equivalent by adding the
        Protobuf message ID offset.
        """
        return legacy_id + MessageId.PROTOBUF_MSG_ID

    @staticmethod
    def from_protobuf(proto_id: int) -> int:
        """
        Converts an incoming Protobuf ID back to its legacy equivalent by
        subtracting the Protobuf message ID offset.
        """
        return proto_id - MessageId.PROTOBUF_MSG_ID
